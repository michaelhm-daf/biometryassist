...
#' @param spaces_in_groups Logical (default `FALSE`). Include monospaced letters in grouping so that the grouping letters line up with a monospaced type font?
...
multiple_comparisons <- function(model.obj,
    ...
    if (groups) {
        pp <- add_letter_groups(pp, diffs, descending, spaces_in_groups)
    }
...
add_letter_groups <- function(pp, diffs, descending, spaces_in_groups) {
    ll <- multcompView::multcompLetters3("Names", "predicted.value", diffs, pp, reversed = !descending)
    if(spaces_in_groups==TRUE){
